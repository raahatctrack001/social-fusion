...
const EditPost = ({ placeholder }) => {
    const postData = JSON.parse(localStorage.getItem("postToUpdate"));
    console.log("data from local storage", postData)
  ...
  const [thumbnailURL, setThumbnailURL] = useState(postData?.thumbnail.at(-1)||'');
  ...
  const handlePostUpdate = async (e)=>{      
      ...
      try {
        ...
        const response = await fetch(apiEndPoints.updatePostAddress(postData?._id), {
            method: "PUT",
            body: formData,
        })
    ...
    <div className='w-full bg-gray-300 border-2 border-rose-900 md:px-10 rounded-lg'>
      ...
      <div className='m-5 min-h-screen'>
          ...
          <div>
            ...
            <Button onClick={()=>thumbnailRef.current.click()} className='w-full hover:bg-gray-500' color={''}  outline> Update Thumbnail Image </Button>
          </div>
          <div className=''>
            
            {showInstructions && <div className="p-4 bg-blue-100 border border-blue-300 rounded-lg mb-1">
              <div className='flex justify-between'>
                <h2 className="text-lg font-semibold text-blue-800 mb-2">Update Images to Your Post</h2> 
