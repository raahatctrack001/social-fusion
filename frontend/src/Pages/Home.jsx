...
const Home = () => {
  ...
  <div className='flex flex-nowrap gap-4 flex-col md:flex-row mx-2 px-4 white justify-center'>
    ...
    <div className='flex flex-col h-[750px] overflow-scroll'>
    {/* {postData ? <ShowPosts heading={`Our recent posts page ${currentPage}/${totalPost}`} postData={postData} /> : <NotFoundPage /> } */}
      ...
      <div className='w-full flex items-center justify-center gap-4 mb-4'> 
          <Button disabled={currentPage <= 1} onClick={()=>setCurrentPage(currentPage=>currentPage-1)}> Prev </Button> 
            {currentPage-3 > 0 && 
              <div className='flex gap-2 cursor-pointer hover:bg-gray-500 hover:text-white px-1 rounded-lg' onClick={()=>setCurrentPage(1)}> 
                <span > 1 </span>
                <span> . </span>
                <span> . </span>
                <span> . </span>              
              ...
              <span onClick={()=>setCurrentPage(currentPage-2)} className='cursor-pointer hover:bg-gray-500 hover:text-white px-1 rounded-lg'>{currentPage-2 > 0 && currentPage-2}</span> 
              <span onClick={()=>setCurrentPage(currentPage-1)} className='cursor-pointer hover:bg-gray-500 hover:text-white px-1 rounded-lg'>{currentPage-1 > 0 && currentPage-1}</span> 
              ...
              <span className='cursor-pointer hover:bg-gray-500 hover:text-white px-1 rounded-lg' onClick={()=>setCurrentPage(currentPage+1)}>{(currentPage+1 <= totalPost) && currentPage+1} </span>
              <span className='cursor-pointer hover:bg-gray-500 hover:text-white px-1 rounded-lg' onClick={()=>setCurrentPage(currentPage+2)}>{(currentPage+2 <=  totalPost) && currentPage+2}</span>
              {currentPage+2 < totalPost && 
              <div className='flex gap-2 cursor-pointer hover:bg-gray-500 hover:text-white px-1 rounded-lg' onClick={()=>setCurrentPage(totalPost)}>
                <span> . </span>
                <span> . </span>
                <span> . </span>
                <span> {totalPost} </span>
    ...
    <div className='flex-1/4 m-2 px-2 mx-2'>
      ...
      <div className="flex flex-col bg-gray-100 dark:bg-gray-900 gap-3 h-[650px] overflow-y-scroll mb-5">
      {users && users.map((author, index) => (
        ...
        <div className='w-full flex justify-center items-center gap-1'> 
          <Button disabled={currentUserPage === 1} onClick={()=>setCurrentUserPage(currentUserPage=>currentUserPage-1)}> prev </Button>
           { currentUserPage-3 >0 && <div className='flex justify-center items-center gap-1 cursor-pointer hover:bg-gray-500 hover:text-white px-1 rounded-lg' onClick={()=>setCurrentUserPage(1)}>
              <span> 1 </span>
              <span> . </span>
              <span> . </span>
              <span> . </span>
            ...
            <div> 
              <span className='cursor-pointer hover:bg-gray-500 hover:text-white px-1 rounded-lg' onClick={()=>setCurrentUserPage(currentUserPage-2)}> {currentUserPage-2 > 0 && currentUserPage-2}</span> 
              <span className='cursor-pointer hover:bg-gray-500 hover:text-white px-1 rounded-lg' onClick={()=>setCurrentUserPage(currentUserPage-1)}> {currentUserPage-1 > 0 && currentUserPage-1}</span> 
            </div>
            <div className=' w-12 flex justify-center items-center font-bold bg-gray-700 text-white rounded-lg py-1'> {currentUserPage} </div>
            <div> 
              <span className='cursor-pointer hover:bg-gray-500 hover:text-white px-1 rounded-lg' onClick={()=>setCurrentUserPage(currentUserPage+1)}>{currentUserPage+1 <= totalUser && currentUserPage+1} </span>
              <span className='cursor-pointer  hover:bg-gray-500 hover:text-white px-1 rounded-lg' onClick={()=>setCurrentUserPage(currentUserPage+2)}>{currentUserPage+2 <= totalUser && currentUserPage+2} </span>
            </div>
            {currentUserPage+3 <= totalUser && <div className='flex justify-center items-center gap-1 cursor-pointer hover:bg-gray-500 hover:text-white px-1 rounded-lg' onClick={()=>setCurrentUserPage(totalUser)}>
              <span> . </span>
              <span> . </span>
              <span> . </span>
              <span> {totalUser} </span>
