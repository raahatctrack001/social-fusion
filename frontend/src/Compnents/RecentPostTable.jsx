          ...
          {displayPosts && displayPosts.length > 0 ? (
            displayPosts.map((post) => (
              <Table.Row key={post._id} className=''>
                <Table.Cell className='text-blue-700 cursor-pointer' onClick={()=>{navigate(`/posts/post/${post?._id}`)}}>{post?.title}</Table.Cell>
                <Table.Cell className='text-blue-700 cursor-pointer' onClick={()=>{navigate(`/posts/post/${post?._id}`)}}>
                    <DisplayContent content=   {post?.content.length > 50 ? `${post?.content.slice(0, 50)}...` : post?.content} />
                </Table.Cell>
                <Table.Cell className='text-blue-700 cursor-pointer text-nowrap' onClick={()=>navigate(`/authors/author/${post?.author?._id}`)}>{post?.author?.fullName?.length > 10 ? post?.author?.fullName.substr(0, 6)+"...": post?.author?.fullName}</Table.Cell>
